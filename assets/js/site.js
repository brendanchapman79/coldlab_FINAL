// ── DARK MODE ────────────────────────────────────────────────────────────────
// Runs immediately (before DOM ready) to prevent flash of wrong theme.
// Priority: 1) user's saved preference, 2) OS preference
...
function initTheme() {
  ...
  applyTheme(saved || osPrefers);

  // Live-update if OS preference changes while page is open (no saved override)
...
initTheme(); // run immediately — before any partial injection
...
async function injectPartial(id, path) {
  try {
    ...
    // Wire theme button after nav injection
